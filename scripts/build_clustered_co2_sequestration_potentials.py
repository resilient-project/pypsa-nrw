...
def allocate_sequestration_potential(
    ...
    overlay["geometry"] = overlay.geometry.buffer(0)
    ...
    buffer_shapes["geometry"] = buffer_shapes.apply(
        lambda x: x["geometry"].buffer(20*1e3), axis=1
    )
    # Explode
    ...
    buffer_shapes["id"] = buffer_shapes["name"] + " co2_stor_offshore " + buffer_shapes["id"].astype(str) if buffer_shapes["id"].max() > 0 else buffer_shapes["name"]
...
if __name__ == "__main__":
    ...
    # gdf with geometries from points x,y in s
    gdf = gpd.GeoDataFrame(
        s,
        geometry=gpd.points_from_xy(s.x, s.y),
        crs="EPSG:4326",
    )
